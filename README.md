...
![金融情報](docs/screenshots/secrets.png)

## 生存確認の方法

本アプリは、一定期間（デフォルト 168 時間）生存確認がないと作動（メール送信）します。定期的に以下のいずれかの方法で生存確認を行ってください。

| 方法 | 手間 | 向いている場面 |
|------|------|----------------|
| ダッシュボードから | ログインが必要 | 普段使っている PC から |
| 専用 URL から | **URL を開くだけ・ログイン不要** | スマホ・タブレットなど他の端末から |

### ダッシュボードから

ログイン後のダッシュボードで「次回までの間隔（時間）」を指定し、「今すぐ生存確認」を押します。確認期限が現在時刻から指定時間後に延長されます。

### 専用 URL から（推奨）

「設定」画面の「秘密ページによる生存確認」で専用 URL を発行できます。この URL にアクセスするだけで、設定済みの確認間隔を保ったまま生存確認日時だけが更新され、完了画面が表示されます。

URL は `https://<ホスト>:<ポート>/checkin/<トークン>` の形式です。日々の生存確認はブラウザのブックマークからこの URL を開くだけで済みます。

- URL を知っている人は誰でも生存確認を更新できるため、パスワードと同じように秘密として扱ってください。
- URL が漏れた恐れがある場合は、設定画面の「URL を再生成」で即座に無効化できます。
- 誤って生存確認を忘れないよう、スマホのリマインダーやカレンダーの繰り返し通知と組み合わせるのが効果的です。

作動の流れやセキュリティ上の注意の詳細は、[ユーザーマニュアル](docs/user-manual.md) を参照してください。
